--[[
	...
	A deliberately plain, functional first-draft UI so the game is playable
	and publishable this week. It is built entirely in code (no pre-made
	UI in Studio) so it works the moment you sync with Rojo -- swap in your
	own art, fonts, and layout once the loop feels fun. Nothing here is
...
local function updateCardButtonVisual(index)
	local button = cardButtons[index]
	if not button then
		return
	end
	if selected[index] then
		button.BackgroundColor3 = Color3.fromRGB(255, 214, 130)
	else
